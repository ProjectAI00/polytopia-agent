/**
 * llmPlay.ts — Legacy LLM bot. Use bot.ts (BOT_MODE=llm) instead.
...
import { askLLM as callLLM, getLLMInfo } from "./llmBackend.js";
...
async function askLLM(prompt: string, numActions: number): Promise<number> {
  ...
  try {
    ...
    const choice = parseInt(parsed.choice ?? "1") - 1;
...
async function main() {
  console.log(`LLM bot using ${getLLMInfo()}\nWaiting for bot turn...\n`);
